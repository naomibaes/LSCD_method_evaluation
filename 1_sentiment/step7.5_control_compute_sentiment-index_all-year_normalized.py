...
def extract_file_details(filename):
    ...
    # Extract injection ratio (numbers after 'sentiment_')
    match_inj = re.search(r"sentiment_(\d+)", filename)
...
def process_collocates(input_dir, output_dir, warriner_ratings):
    ...
    if results:
        ...
        pivot_df.to_csv(os.path.join(output_dir, 'control_averaged_valence_index_all-year_normalized.csv'), index=False)
    else:
