...
input_dirs = {
    "positive": os.path.join(base_dir, "input", "positive"),
    "negative": os.path.join(base_dir, "input", "negative")
}
output_dirs = {
    "positive": os.path.join(base_dir, "positive"),
    "negative": os.path.join(base_dir, "negative")
}
...
def process_target(target):
    ...
    for condition in ["positive", "negative"]:
        natural_path = os.path.join(input_dirs[condition], "natural", f"{target}_all-year_sentiment_natural.tsv")
        synthetic_path = os.path.join(input_dirs[condition], "synthetic", f"{target}_all-year_sentiment_synthetic.tsv")
        ...
        for round_num in range(1, NUM_ROUNDS + 1):  # 6 rounds
            ...
            for repeat, (natural_sample, synthetic_sample) in enumerate(zip(natural_samples, synthetic_samples), start=1):
                output_path = os.path.join(output_dirs[condition], f"{target}_synthetic_sentiment_50_round{round_num}.{repeat}.tsv")
