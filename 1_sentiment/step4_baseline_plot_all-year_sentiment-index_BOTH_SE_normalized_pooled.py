...
# Load the data
df = pd.read_csv("output/baseline_averaged_valence_index_all-year_normalized_pooled.csv")
df['injection_ratio'] = pd.to_numeric(df['injection_ratio'], errors='coerce')

# Remove '_synthetic_sentiment' from the target column
df['target'] = df['target'].str.replace('_synthetic_sentiment', '', regex=False)
...
y_ticks = np.arange(0.48, 0.62, 0.02)
...
plt.savefig("../figures/plot_all-year_sentiment_BOTH_baseline_with_SE_normalized_pooled.png", dpi=600, bbox_inches='tight')
plt.show()
