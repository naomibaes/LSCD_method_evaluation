...
df = pd.read_csv("output/absa_averaged_valence_index_all-year.csv")
...
for term in target_terms:
    ...
    if not term_data.empty:
        min_vals.append(term_data[['avg_valence_index_positive', 'avg_valence_index_negative']].min().min())
        max_vals.append(term_data[['avg_valence_index_positive', 'avg_valence_index_negative']].max().max())

if min_vals and max_vals:
    overall_min = min(min_vals)
    overall_max = max(max_vals)
    y_ticks = np.arange(start=np.floor(overall_min*20)/20, stop=np.ceil(overall_max*20)/20, step=0.1)  # Set step for ticks

# Plot Positive Valence Index (Top Panel)
for term in target_terms:
    term_data = df[df['target'] == term]
    if not term_data.empty:
        term_data = term_data.sort_values(by='injection_ratio')
        ax1.errorbar(term_data['injection_ratio'], term_data['avg_valence_index_positive'],
                     yerr=term_data['se_valence_index_positive'], fmt='o', color=colors[term],
                     capsize=4, markersize=6)
        ax1.plot(term_data['injection_ratio'], term_data['avg_valence_index_positive'],
                 color=colors[term], linestyle='-')

# Plot Negative Valence Index (Bottom Panel)
for term in target_terms:
    term_data = df[df['target'] == term]
    if not term_data.empty:
        term_data = term_data.sort_values(by='injection_ratio')
        ax2.errorbar(term_data['injection_ratio'], term_data['avg_valence_index_negative'],
                     yerr=term_data['se_valence_index_negative'], fmt='s', color=colors[term],
                     capsize=4, markersize=6)
        ax2.plot(term_data['injection_ratio'], term_data['avg_valence_index_negative'],
                 color=colors[term], linestyle='--')
...
ax1.axvspan(10, 90, color='grey', alpha=0.2)
ax2.axvspan(10, 90, color='grey', alpha=0.2)
ax1.axvspan(90, 105, color='darkgrey', alpha=0.5)
...
ax1.set_ylabel('Valence Score', fontsize=font_label)
ax2.set_ylabel('Valence Score', fontsize=font_label)
...
plt.savefig("../figures/plot_all-year_sentiment_BOTH_absa.png", dpi=600, bbox_inches='tight')
plt.show()
