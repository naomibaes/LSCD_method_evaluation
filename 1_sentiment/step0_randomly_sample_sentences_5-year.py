...
def process_target_and_interval(args):
    ...
    synthetic_file_path = os.path.join(synthetic_corpus_dir, f"{target}_{start}-{end}.synthetic_1500_sentences")
    ...
    for repeat in range(1, NUM_REPEATS + 1):
        for ratio in injection_ratios:
            ...
            # Replace the 'type' column for synthetic rows with the sibling label
            synthetic_sample = synthetic_sample.rename(columns={"type": "sibling_label"})
            synthetic_sample["type"] = synthetic_sample["sibling_label"]
            synthetic_sample = synthetic_sample.drop(columns=["sibling_label"])
            ...
            output_file_path = os.path.join(output_dir, f"{target}_{interval_label}_synthetic_{int(ratio * 100)}.{repeat}")
            combined_sample.to_csv(output_file_path, index=False, header=True, sep='\t')
