...
targets = ["mental_health", "mental_illness", "mental_disorder", "trauma", "addiction", 
           "bullying", "harassment", "abuse", "anger", "distress", "empathy", "grief", 
           "anxiety", "depression", "stress", "worry", "automobile", "bank", "camera", "car", "deer", 
...
fig, axs = plt.subplots(n_rows, n_cols, figsize=(8, n_rows * 2.5), constrained_layout=True)
...
for index, (target, corpus) in enumerate(valid_axes):
    ...
    # Group by year without any interval
    ...
    ax.bar(grouped_df['year'], grouped_df['count'], color=color, edgecolor='darkgrey')
    
    # Simplified title with term label only
    ax.set_title(f"{target.replace('_', ' ').capitalize()}", fontsize=10)
    ax.set_xlabel('Year', color='black', fontsize=8)
    ax.set_ylabel('Sentence Count', color='black', fontsize=8)

    # Set x-axis ticks and labels for each year, rotate labels for better readability
    ax.set_xticks(grouped_df['year'][::5])  # Show every 5th year for readability
    ax.set_xticklabels(grouped_df['year'][::5], rotation=30, ha="right", fontsize=7)
...
output_file_path = f"{output_directory}/1-year_counts_sentences.png"
