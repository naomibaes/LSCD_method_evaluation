...
# Load the dataset
try:
    df = pd.read_csv("output/control_averaged_arousal_index_5-year_normalized.csv")
...
for i, term in enumerate(target_terms):
    ...
    for injection_ratio in sorted(df['injection_ratio'].unique(), reverse=True):
        ...
        if not data.empty:
            ...
            # High Intensity: Always Dashed (`--`)
            y_values_pos = data['avg_arousal_index_high']
            color = adjust_color_shade(colors[term], injection_ratio)
            ax.plot(x_values, y_values_pos, linestyle='--', marker='o', color=color, label=f'High (Ratio {injection_ratio})')

            # Negative Intensity: Always Dotted (`:`)
            y_values_neg = data['avg_arousal_index_low']
            ax.plot(x_values, y_values_neg, linestyle=':', marker='x', color=color, label=f'Low (Ratio {injection_ratio})')
...
plt.savefig("../figures/plot_5-year_intensity_BOTH_control_normalized.png", dpi=600, bbox_inches='tight')
plt.show()
