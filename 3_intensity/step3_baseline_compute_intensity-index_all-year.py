...
def compute_arousal_indices(collocates_df, warriner_ratings):
    """Compute arousal indices after normalizing and merging collocate data with Warriner ratings."""
    ...
    if not merged_data.empty:
        ...
        variance = np.var(merged_data['A.Mean.Sum'], ddof=1) * total_count
...
def process_collocates(input_dir, output_dir, warriner_ratings, targets):
    results = []
    arousal_indices = {}  # Dictionary to store arousal indices for SEM calculation
    ...
    for file in tqdm(all_files, desc='Processing files'):
        ...
        if target and injection_ratio:
            arousal_index, _ = compute_arousal_indices(collocates_df, warriner_ratings)  # Ignore SEM here
            results.append({
                'target': target,
                'injection_ratio': injection_ratio,
                f'avg_arousal_index_{subfolder}': arousal_index
            })
            key = (target, injection_ratio, subfolder)
            if key not in arousal_indices:
                arousal_indices[key] = []
            arousal_indices[key].append(arousal_index)

    # Calculate SEM after all files have been processed
    for key, indices in arousal_indices.items():
        ...
        for result in results:
            if result['target'] == target and result['injection_ratio'] == injection_ratio:
                result[f'se_arousal_index_{subfolder}'] = sem

    if results:
        results_df = pd.DataFrame(results)
        final_df = results_df.groupby(['target', 'injection_ratio']).agg({
            'avg_arousal_index_high': 'mean',
            'avg_arousal_index_low': 'mean',
            'se_arousal_index_high': 'mean',
            'se_arousal_index_low': 'mean'
        }).reset_index()
        final_df.to_csv(os.path.join(output_dir, 'baseline_averaged_arousal_index_all-year.csv'), index=False)
        print(f"Output written to: {os.path.join(output_dir, 'baseline_averaged_arousal_index_all-year.csv')}")
    else:
