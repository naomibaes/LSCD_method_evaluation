...
# Load the data
df = pd.read_csv("output/baseline_averaged_arousal_index_all-year_normalized.csv")
df['injection_ratio'] = pd.to_numeric(df['injection_ratio'], errors='coerce')

# Remove '_synthetic_intensity' from the target column
df['target'] = df['target'].str.replace('_synthetic_intensity', '', regex=False)
...
plt.savefig("../figures/plot_all-year_intensity_BOTH_baseline_with_SE_normalized.png", dpi=600, bbox_inches='tight')
plt.show()
