...
null_model = smf.mixedlm("avg_arousal_index_low ~ 1", data, groups=data['target'], re_formula="1").fit()
...
full_model = smf.mixedlm("avg_arousal_index_low ~ injection_ratio", data, groups=data['target'], re_formula="1").fit()
print("Full Model Summary:")
print(full_model.summary())

# Model comparison with and without 'epoch'
model_with_epoch = smf.mixedlm("avg_arousal_index_low ~ injection_ratio * epoch", data, groups=data['target'], re_formula="1").fit()
...
levene_test = levene(data['avg_arousal_index_low'], fitted)
...
ols_model = smf.ols("avg_arousal_index_low ~ injection_ratio", data=data).fit()
