...
try:
    df = pd.read_csv("output/baseline_averaged_arousal_index_5-year_normalized.csv")
...
for i, term in enumerate(target_terms):
    ...
    for injection_ratio in sorted(df['injection_ratio'].unique(), reverse=True):
        ...
        if not data.empty:
            ...
            ax.plot(x_values, y_values_pos, linestyle=linestyle_high, marker='o', color=color)
...
# Define the legend for High and Negative Intensity
legend_lines = [
...
plt.savefig("../figures/plot_5-year_intensity_BOTH_baseline_normalized.png", dpi=600, bbox_inches='tight')
plt.show()
