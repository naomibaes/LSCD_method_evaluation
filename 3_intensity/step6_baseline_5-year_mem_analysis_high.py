...
# Load your data
data = pd.read_csv("output/baseline_averaged_arousal_index_5-year.csv")
...
null_model = smf.mixedlm("avg_arousal_index_high ~ 1", data, groups=data['target'], re_formula="1").fit()
...
full_model = smf.mixedlm("avg_arousal_index_high ~ injection_ratio", data, groups=data['target'], re_formula="1").fit()
print("Full Model Summary:")
print(full_model.summary())

# Model comparison with and without 'epoch'
model_with_epoch = smf.mixedlm("avg_arousal_index_high ~ injection_ratio * epoch", data, groups=data['target'], re_formula="1").fit()
...
levene_test = levene(data['avg_arousal_index_high'], fitted)
...
ols_model = smf.ols("avg_arousal_index_high ~ injection_ratio", data=data).fit()
