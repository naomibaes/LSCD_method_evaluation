...
def get_average_dissimilarity_score(file_manager, inputs_to_embeddings, injection_level, target, iteration, year=None):
...
def main(data_folder):
    ...
    inputs_to_embeddings = file_manager.get_inputs_to_embeddings(pickle_filename=f'inputs_to_embeddings_{args.year_mode}.pkl')
    ...
    for target in targets:
        ...
        if args.year_mode == '5-year':
            for year in YEARS:
                for injection_level in INJECTION_LEVELS:
                    ...
                    for iteration in ITERATION_LEVELS:
                        dissimilarity_scores_per_injection.append(get_average_dissimilarity_score(file_manager, inputs_to_embeddings, injection_level, target, iteration, year))
            ...
            for injection_level in INJECTION_LEVELS:
                dissimilarity_scores_per_injection = []
                for iteration in ITERATION_LEVELS:
                    dissimilarity_scores_per_injection.append(get_average_dissimilarity_score(file_manager, inputs_to_embeddings, injection_level, target, iteration))
                avg_dissimilarity = np.mean(dissimilarity_scores_per_injection)
                results.append({
                    "dimension": "breadth",
                    "variation": "N/A",
    ...
    filename = f'breadth_lexeme_dissimilarity_{args.year_mode}.csv'
    save_to_csv(results, filename)


data_folder = f"output/{args.year_mode}.cosine"
