        ...
        if os.path.isdir(data_folder):
            for file_name in os.listdir(data_folder):
                ...
                if os.path.isfile(file_path):
                    ...
                    try:
                        # Open TSV file correctly
                        with open(file_path, "r", encoding="utf-8") as f:
                            reader = csv.reader(f, delimiter="\t")  # **Explicitly set to TSV**
                            sentences = [row[0] for row in reader if len(row) > 0]  # Handle possible empty lines
...
data_folders = [
    "output/all-year.cosine/control"
]
