...
def process_file(file_path):
    ...
    pattern = re.compile(r"(" + '|'.join(target_terms) + r")_(\d{4}-\d{4})_synthetic_(\d+)_\d+_cds_mpnet")
...
def main(folder_path):
    ...
    for file_name in all_files:
        # Ensure only files ending with "mpnet" are processed
        if file_name.endswith("mpnet"):
    ...
    combined_output_filename = "output/baseline_final_combined.5-year.cds_mpnet.csv"
