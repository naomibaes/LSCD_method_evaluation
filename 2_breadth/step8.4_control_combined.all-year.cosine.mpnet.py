...
def process_file(file_path):
    ...
    # Updated regex to correctly extract term, round number, and injection ratio
    pattern = re.compile(r"(\w+)_synthetic_breadth_(\d+)_round(\d+)\.(\d+)\.tsv_cds_mpnet")
    ...
    term, inj_ratio, round_number, repeat_number = match.groups()
    ...
    epoch = "all-year"  # Assuming constant epoch for these files
    corpus = "synthetic_breadth"

    print(f"Processing file: {file_name} | Term: {term} | Round: {round_number} | Repeat: {repeat_number} | Inj Ratio: {inj_ratio}")
...
def main(folder_path):
    ...
    output_filename = "output/control_final_combined.all-year.cds_mpnet.csv"
...
# Run the processing
folder_path = "output/all-year.cosine/control"
main(folder_path)
