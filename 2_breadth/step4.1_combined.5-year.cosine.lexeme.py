...
def process_file(file_path):
    ...
    # Create a pattern that matches file names correctly handling compound terms
    pattern = re.compile(r"(" + '|'.join(target_terms) + r")_(\d{4}-\d{4})_synthetic_(\d+)_\d+_cds_lexeme")
...
def main(folder_path):
    ...
    for file_name in all_files:
        # Ensure only files ending with "lexeme" are processed
        if file_name.endswith("lexeme"):
    ...
    combined_output_filename = "output/baseline_final_combined.5-year.cds_lexeme.csv"
