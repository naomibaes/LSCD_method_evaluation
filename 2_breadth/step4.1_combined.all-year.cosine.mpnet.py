...
def process_file(file_path):
    ...
    match = re.match(r"(\w+)_synthetic_breadth_(\d+)_(\d+).csv_cds_mpnet", file_name)
    if not match:
...
def main(folder_path):
    ...
    for file_name in all_files:
        ...
        if os.path.isfile(file_path) and "cds_mpnet" in file_name:
    ...
    combined_output_filename = "output/baseline_final_combined.all-year.cds_mpnet.csv"
